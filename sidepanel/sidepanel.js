...
function updateAccountRunHistorySettingsUI() {
  ...
  rowAccountRunHistoryHelperBaseUrl.style.display = inputAccountRunHistoryTextEnabled.checked
    ? ''
    : 'none';
}
...
async function restoreState() {
  try {
    ...
    if (latestState?.heroSmsApiKey && normalizeHeroSmsBaseUrlValue(latestState?.heroSmsBaseUrl || '')) {
...
function getLocalVersionLabel() {
  const manifest = chrome.runtime.getManifest?.() || {};
  return manifest.version_name || (manifest.version ? `v${manifest.version}` : 'Pro0.0');
}

function initializeVersionInfo() {
  if (extensionUpdateStatus) {
    extensionUpdateStatus.textContent = getLocalVersionLabel();
  }
