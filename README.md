...
当前版本基于侧边栏控制，支持单步执行、整套自动执行、停止当前流程、保存常用配置，以及通过 DuckDuckGo / QQ / 163 / Inbucket / Hotmail 协助获取邮箱验证码，并可通过 Hero-SMS 自动完成手机号验证。
...
- 支持 `Hero-SMS`：自动获取手机号、提交手机号、轮询短信验证码、处理重发、失败换号、取消激活和号码复用
- Step 5 同时兼容两种页面：
  - 页面要求填写 `birthday`
  - 页面要求填写 `age`
- 支持 `Auto` 多轮运行
...
- 如果需要自动完成手机号验证，需要准备 Hero-SMS API Key，并确认服务编号、国家 ID、最高价格等配置可用
...
当 OAuth 登录后进入 OpenAI 手机号验证页时，可使用 Hero-SMS 自动接码。

1. 在侧边栏填写 Hero-SMS API 地址，默认通常为 `https://hero-sms.com/stubs/handler_api.php`
2. 填写 Hero-SMS API Key
3. 填写服务编号，例如 OpenAI/ChatGPT 常用服务编号可配置为 `dr`
4. 填写国家 ID，例如 `52`
5. 可选填写最高价格 `maxPrice`，用于限制取号价格
6. 先确认 Hero-SMS 当前服务和国家有库存，再运行到手机号验证步骤
7. 自动流程检测到手机号页后，会获取或复用可用号码、提交手机号、等待短信验证码并自动回填
8. 遇到号码不可用、短信超时、重发受限等情况时，会按内置策略释放或加入备选池，并尝试新号码
...
- `Hero-SMS API 地址`：Hero-SMS handler API 地址，默认可使用 `https://hero-sms.com/stubs/handler_api.php`
- `Hero-SMS API Key`：你的 Hero-SMS API 密钥
- `服务编号`：Hero-SMS 服务代码，例如 `dr`
- `国家 ID`：Hero-SMS 国家数字 ID，例如 `52`
- `最高价格`：可选，传给 `getNumberV2` 的 `maxPrice`

当前使用到的 Hero-SMS 接口：

- `getNumberV2`：获取新号码
- `getActiveActivations`：同步远端活动激活列表，避免提交已失效号码
- `getStatusV2`：轮询短信状态并读取验证码
- `setStatus=1`：标记激活准备接收短信
- `setStatus=3`：请求新短信或重发
- `setStatus=6`：完成激活
- `setStatus=8`：取消激活

手机号验证行为：

- 检测到手机号页面后，优先复用当前可用号码
- 当前号码不可用时，会尝试备用号码、远端活动号码，最后才获取新号码
- 手机号提交成功后，会等待 5 秒再调用 `setStatus=3`
- 页面“重新发送验证码”按钮点击成功后，也会等待 5 秒再调用 `setStatus=3`
- 等待验证码期间每 5 秒轮询一次 `getStatusV2`
- 60 秒仍未收到新验证码时会请求重发
- 180 秒仍未收到验证码时会按超时失败处理
- 验证码被页面拒绝时，不重新提交手机号，只对同一号码 `setStatus=3` 等待不同的新验证码
- 号码失败后会等待 2 分钟再尝试 `setStatus=8` 取消激活；页面提示无法向号码发送短信时会先加入失败列表并由后台延迟清理；如果 Hero-SMS 返回 409，例如 `EARLY_CANCEL_DENIED` 或 `OTP_RECEIVED`，该号码会进入备选池
